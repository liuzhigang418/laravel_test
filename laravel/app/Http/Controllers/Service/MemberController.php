    ...
    public function register(Request $request)
    {
        ...
        if ($phone != '') {
            ...
            if ($tempPhone->code == $phone_code) {
                ...
                $registerPhone = Member::where('phone', $phone)->first();
                if ($registerPhone) {
                    $m3_result->status = 7;
                    $m3_result->message = '该手机号已注册';
                    return $m3_result->toJson();
                }
                $member = new Member;
                $member->phone = $phone;
//        $member->
            ...
            $m3_email->to = $email;
            $m3_email->cc = '979399843@qq.com';
            $m3_email->subject = '凯恩书店验证';
            $m3_email->content = '请于24小时点击该链接完成验证.laravel.com:8080/service/validate_email'
                . '?member_id=' . $member->id
                . '&code=' . $uuid;
    ...
    function login(Request $request){
        ...
        $validate = $request->get('validate_code','');
        ...
        if($validate != $validate_code_session){
            $m3_result->status = 1;
            $m3_result->message='验证码不正确';
            return $m3_result->toJson();
        }
